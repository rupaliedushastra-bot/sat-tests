// ── 240 Country Codes & Dialing Codes ────────────────────────────────────────
const COUNTRY_CODES = [
...
function populateCountrySelects() {
  ...
  selects.forEach(select => {
    ...
    // Quick access top codes
    const topCodes = ['US', 'IN', 'GB', 'CA', 'AU'];
    const addedTop = new Set();
    
    topCodes.forEach(iso => {
      const country = COUNTRY_CODES.find(c => c.iso === iso);
      if (country) {
        const opt = document.createElement('option');
        opt.value = country.code;
        opt.textContent = `${country.iso} (${country.code})`;
        opt.title = country.name;
        if (iso === 'US') opt.selected = true; // Default to US (+1)
