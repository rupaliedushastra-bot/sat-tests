...
// =============================================
// script.js – Transitions
// Digital SAT Practice | EduQuest
// 36 Questions | Rhetorical Skills
// =============================================

async function saveUserToSupabase(student) {
    if (typeof window !== 'undefined' && window._masterSaveRegistration) {
        return await window._masterSaveRegistration(student, 'Transitions');
    }
}

async function saveToSupabase(result) {
    if (!result.examName) result.examName = 'Transitions';
    if (!result.topicNumber) result.topicNumber = 21;
...
function renderResults(result, saveRes) {
  ...
  lastResultDetails = result.details || [];
    ...
    wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px">Transitions · Topic Test</div>
