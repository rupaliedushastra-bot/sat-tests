  ...
  try {
    await fetch(`${SUPABASE_URL}/rest/v1/sat_topic_user`, {
      ...
      body: JSON.stringify({
        ...
        email: student.email,
        phone: student.phone,
        topic: 'Research Notes Questions',
...
function renderQ() {
  ...
  if ($('qTopic')) $('qTopic').textContent = q.topic || 'Research Notes Questions';
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Topic 8 · Research Notes Questions</div>
...
function downloadPDF() {
  ...
  const opt = {
    margin:       [10, 10, 10, 10],
    filename:     `EduQuest_Report_${student.name.replace(/[^a-zA-Z0-9]/g, '_')}_Topic8.pdf`,
