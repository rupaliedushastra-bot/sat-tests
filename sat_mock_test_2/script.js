...
function generateCleanReportHTML(t, details, emailStr, studentNameStr) {
  ...
  const testTitle = t.exam_name || t.topic || 'Digital SAT Mock Test 2';
...
function renderQ() {
  ...
  const modNum = q.module || (q.id <= 27 ? 1 : 2);
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Digital SAT Mock Test 2</div>
...
function downloadPDF() {
  ...
  const opt = {
    margin: [10, 10, 10, 10],
    filename: `SAT_Mock_Test_2_Report_${sName.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`,
