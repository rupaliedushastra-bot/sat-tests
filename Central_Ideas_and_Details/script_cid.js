...
// =============================================
// script_cid.js – Central Ideas and Details Topic Test
// Digital SAT Practice | EduQuest
// Updated: Supports 5-option (A-E) questions & Set 3 passage display
// =============================================

async function saveUserToSupabase(student) {
  if (typeof window !== 'undefined' && window._masterSaveRegistration) {
    return await window._masterSaveRegistration(student, 'Central Ideas and Details');
  }
}

async function saveToSupabase(result) {
  if (!result.examName) result.examName = 'Central Ideas and Details';
  if (!result.topicNumber) result.topicNumber = 5;
...
// ── Passage Display Helper ────────────────────
// For Set 3 questions that have a `passage` field, we display the passage
// in a styled box above the question stem.
function buildPassageHTML(q) {
  if (!q.passage) return '';
  // Extract the question stem (the part after the last \n\n)
  // The passage field has the full passage; q.text has passage + question.
  // We use q.passage to render a styled block, and derive the stem separately.
  return `
...
function renderQ() {
  ...
  if ($('qTopic')) $('qTopic').textContent = q.topic || 'Central Ideas and Details';
  ...
  // Build the question text display
  // For Set 3 passage questions, display passage separately then the stem
  const qTextEl = $('qText');
  if (q.passage) {
    // Extract stem: everything after the passage block in q.text
    // The stem is the last paragraph (starts with "Q" number and colon)
    const stemMatch = q.text.match(/\nQ\d+:[\s\S]+$/);
    const stem = stemMatch ? stemMatch[0].trim().replace(/^Q\d+:\s*/, '') : '';
    qTextEl.innerHTML = buildPassageHTML(q) +
      `<div class="q-stem" style="font-size:1rem;font-weight:600;margin-top:4px;">${stem}</div>`;
  } else {
    qTextEl.textContent = q.text;
  }

  // Build options (supports A–E, i.e. up to 5 options)
  const opts = $('qOpts');
  opts.innerHTML = '';
...
async function doSubmit() {
  ...
  const result = {
    ...
    total, pct, grade, scaled, details,
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Central Ideas and Details · Topic Test</div>
  ...
  const bColors = bands.map((b, i) => {
    ...
    return p >= 0.7 ? '#059669' : p >= 0.4 ? '#d97706' : '#dc2626';
  });
  ...
  new Chart($('barChart'), {
    ...
    data: {
      ...
      datasets: [{
        label: 'Correct', data: bScores,
        backgroundColor: bColors.map(c => c + '33'),
        borderColor: bColors, borderWidth: 2, borderRadius: 6
      }]
    },
    options: {
      plugins: { legend: { display: false } },
      scales: {
        y: { max: maxTotal, ticks: { stepSize: 1, font: { family: 'IBM Plex Mono' } } },
