...
function startTest() {
  ...
  buildPalette();
  renderQ();
  startTimer(35 * 60); // 35 minutes for Topic 14
...
function renderQ() {
  ...
  if ($('qTopic')) $('qTopic').textContent = q.topic || 'Trigonometry';
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Topic 14 · Trigonometry</div>
...
function downloadPDF() {
  ...
  const opt = {
    margin:       [10, 10, 10, 10],
    filename:     `EduQuest_Report_${student.name.replace(/[^a-zA-Z0-9]/g, '_')}_Topic14.pdf`,
