...
function renderQ() {
  ...
  if ($('qTopic')) $('qTopic').textContent = q.topic || 'Inferences';
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Inferences  · Topic Test</div>
...
function downloadPDF() {
  ...
  html2pdf().set({
    margin: [10, 10, 10, 10],
    filename: `EduQuest_Report_${student.name.replace(/[^a-zA-Z0-9]/g, '_')}_PDF_Inferences.pdf`,
