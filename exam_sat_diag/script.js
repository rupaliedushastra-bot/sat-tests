...
// =============================================
// script.js – SAT Diagnostic Test
// Digital SAT Practice | EduQuest
// 38 Questions | Mixed Topics
// =============================================

async function saveUserToSupabase(student) {
    if (typeof window !== 'undefined' && window._masterSaveRegistration) {
        return await window._masterSaveRegistration(student, 'SAT Full Mock Test');
    }
}

async function saveToSupabase(result) {
    if (!result.examName) result.examName = 'SAT Full Mock Test';
    if (!result.topicNumber) result.topicNumber = 23;
...
// Topic colour map for badge
const TOPIC_COLORS = {
    'Subject-Verb Agreement': '#6366f1',
    'Pronoun-Antecedent Agreement': '#8b5cf6',
    'Verb Tense': '#0ea5e9',
    'Verb Form': '#0ea5e9',
    'Dangling Modifiers': '#f59e0b',
    'Plural & Possessives': '#10b981',
    'Linking Clauses': '#ef4444',
    'Supplements (Em-Dashes)': '#ec4899',
    'Supplements (Appositives)': '#ec4899',
    'Supplements (Parentheticals)': '#ec4899',
    'Punctuation': '#14b8a6',
};
...
function renderQ() {
    ...
    const topicTag = $('qTopic');
    if (topicTag) {
        topicTag.textContent = q.topic || 'Mixed';
        const col = TOPIC_COLORS[q.topic] || 'var(--accent)';
        topicTag.style.background = col + '22';
        topicTag.style.color = col;
        topicTag.style.borderColor = col + '55';
    }
...
function renderResults(result, saveRes) {
  ...
  lastResultDetails = result.details || [];
    ...
    // Topic-wise breakdown for diagnostic insight
    const topicGroups = {};
    details.forEach(d => {
    ...
    const weakTopics = Object.entries(topicGroups)
        .filter(([, v]) => (v.correct / v.total) < 0.6)
        .map(([k]) => k);

    const diagnosticInsight = weakTopics.length > 0
        ? `<div class="explanation-box" style="margin:0 0 20px;border-color:rgba(239,68,68,0.3);background:rgba(239,68,68,0.06);">
        <strong>📌 Weak Areas Detected:</strong><br>
        ${weakTopics.map(t => `• ${t}`).join('<br>')}
        <br><br>Focus your revision on these topics to improve your SAT Writing score.
      </div>`
        : `<div class="explanation-box" style="margin:0 0 20px;border-color:rgba(5,150,105,0.3);background:rgba(5,150,105,0.06);">
        <strong>✅ Strong Performance Across All Topics!</strong> Keep practising to maintain this level.
      </div>`;
    ...
    <div class="charts-row">
      ...
      <div class="chart-card"><h3>📈 Topic Performance</h3><canvas id="barChart" height="220"></canvas></div>
    </div>
    ${diagnosticInsight}
