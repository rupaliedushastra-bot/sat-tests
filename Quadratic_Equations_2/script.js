...
function renderQ() {
  ...
  if ($('qTopic')) $('qTopic').textContent = q.topic || 'Quadratic Equations (Set 2)';
  ...
  const imgContainer = $('qImgContainer');
  const imgEl = $('qImg');
  if (imgContainer && imgEl) {
    if (typeof defined_images !== 'undefined' && defined_images[q.id]) {
      imgEl.src = defined_images[q.id];
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Quadratic Equations (Set 2)  · Topic Test</div>
    ...
    <div class="inc-review">
      ...
      ${details.filter(d => d.status !== 'correct').map(q => `
        <div class="inc-q">
          <div class="inc-q-text">Q${q.id}: ${(q.text || '').substring(0, 300).replace(/<[^>]*>/g, '')}${(q.text || '').length > 300 ? '…' : ''}</div>
          ${q.status === 'wrong'
      ...
      : `<div class="inc-row" style="color:var(--amber)"><span class="label">⏭ Skipped</span></div>`}
          ...
          ${(typeof solution_images !== 'undefined' && solution_images[q.id]) ? `<div style="margin-top:10px;text-align:center;"><img src="${solution_images[q.id]}" style="max-width:100%;border-radius:8px;"></div>` : ''}
        </div>`).join('')}
...
function downloadPDF() {
  ...
  html2pdf().set({
    margin: [10, 10, 10, 10],
    filename: `EduQuest_Report_${student.name.replace(/[^a-zA-Z0-9]/g, '_')}_Quadratic_Equations_2.pdf`,
