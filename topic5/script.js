...
async function saveUserToSupabase(student) {
  if (typeof window !== 'undefined' && window._masterSaveRegistration) {
    return await window._masterSaveRegistration(student, 'Rhetorical Skills');
  }
}

async function saveToSupabase(result) {
  if (!result.examName) result.examName = 'Rhetorical Skills';
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Topic 5 · Rhetorical Skills</div>
  ...
  const bands = [
    { label:'Main/Topic Idea', qs: details.filter(d=>['Main Idea','Topic Sentence','Concluding Sentence'].includes(d.topic)) },
    { label:'Evidence/Logic', qs: details.filter(d=>['Supporting Evidence','Strengthen / Weaken'].includes(d.topic)) },
    { label:'Transitions', qs: details.filter(d=>d.topic==='Transitions Between Ideas') },
    { label:'Function', qs: details.filter(d=>d.topic==='Sentence Function') },
  ];
