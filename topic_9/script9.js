...
async function saveUserToSupabase(student) {
  if (typeof window !== 'undefined' && window._masterSaveRegistration) {
    return await window._masterSaveRegistration(student, 'Numbers & Operations');
  }
}

async function saveToSupabase(result) {
  if (!result.examName) result.examName = 'Numbers & Operations';
  if (!result.topicNumber) result.topicNumber = 9;
...
function startTest() {
  ...
  buildPalette();
  renderQ();
  startTimer(35 * 60); // 35 minutes for Topic 9
...
function renderQ() {
  ...
  if ($('qTopic')) $('qTopic').textContent = q.topic || 'Numbers & Operations';
  
  // Image vs Text rendering logic
  const imgWrap = $('qImageWrap');
  const imgEl = $('qImage');
  if (q.useImage && QUESTION_IMAGES[q.imageKey]) {
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Topic 9 · Numbers & Operations</div>
  ...
  new Chart($('barChart'), {
    ...
    options:{ plugins:{ legend:{display:false} },
      scales:{ y:{ max: maxTotal, ticks:{ stepSize:1, font:{family:'IBM Plex Mono'} } },
