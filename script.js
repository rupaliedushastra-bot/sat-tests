...
// Required columns for sat_topic_report: id (uuid), name, email, phone, topic, correct, wrong,
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Topic 1 · Reading Comprehension</div>
    ...
    <div class="res-actions" id="resActions" style="margin-top:24px;">
      ...
      <button class="btn btn-primary" style="width:auto; background:#059669;" onclick="downloadPDF()" id="downloadPdfBtn">📄 Download PDF</button>
    </div>`;
  ...
  const bands = [
    { label:'Main Idea', qs: details.slice(0,5) },
    { label:'Inference', qs: details.slice(5,10) },
    { label:'Evidence', qs: details.slice(10,15) },
    { label:'Tone/Purpose', qs: details.slice(15,20) },
    { label:'Paired Pass.', qs: details.slice(20,25) },
  ];
...
function downloadPDF() {
  ...
  const opt = {
    margin:       [10, 10, 10, 10],
    filename:     `EduQuest_Report_${student.name.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`,
