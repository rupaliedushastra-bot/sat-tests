...
// Populate country codes on load
document.addEventListener('DOMContentLoaded', () => {
  const ccSel = $('regCC');
  if (ccSel && typeof COUNTRY_CODES !== 'undefined') {
    COUNTRY_CODES.forEach(c => {
      const opt = document.createElement('option');
      opt.value = c.code;
      opt.textContent = `${c.iso} (${c.code})`;
      if (c.iso === 'US' || c.code === '+1') opt.selected = true;
...
function generateCleanReportHTML(t, details, emailStr, studentNameStr) {
  ...
  const testTitle = t.exam_name || t.topic || 'Digital SAT Math Practice Test 1';
  ...
  const total = t.total || (details ? details.length : 54);
  ...
  if (details && details.length > 0) {
    qCards = details.map((d, i) => {
      const qId = d.id || (i + 1);
      const modNum = qId <= 27 ? 1 : 2;
      const modQNum = qId <= 27 ? qId : qId - 27;
      ...
      let imgSrc = '';
      if (d.useImage && d.imageKey && typeof QUESTION_IMAGES !== 'undefined' && QUESTION_IMAGES[d.imageKey]) {
        imgSrc = QUESTION_IMAGES[d.imageKey];
      }
      ...
      const expText = d.explanation || 'No detailed explanation available for this question.';
      ...
      return `
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; padding:20px; margin-bottom:20px; box-shadow:0 2px 8px rgba(0,0,0,0.04); page-break-inside:avoid;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px solid #f1f5f9; padding-bottom:10px;">
            <div>
              <span style="font-weight:800; font-size:1rem; font-family:'Courier New', monospace; color:#0f172a;">Question ${qId} (Module ${modNum} · Q${modQNum})</span>
              <span style="display:inline-block; margin-left:8px; font-size:0.75rem; background:#f1f5f9; color:#475569; padding:2px 8px; border-radius:10px;">${d.topic || 'Math'}</span>
            </div>
            ${statusBadge}
          </div>
          
          ${hasText ? `<div style="font-size:0.95rem; line-height:1.65; color:#1e293b; margin-bottom:16px; white-space:pre-wrap;">${qText}</div>` : ''}
          ${imgSrc ? `<div style="text-align:center; margin-bottom:16px;"><img src="${imgSrc}" style="max-width:100%; max-height:260px; border-radius:8px;" alt="Question Diagram" /></div>` : ''}

          <div style="margin-bottom:16px;">
            ${optsHTML}
          </div>

          <div style="padding:12px 16px; background:#f0f9ff; border-left:4px solid #0284c7; border-radius:0 8px 8px 0; font-size:0.88rem; line-height:1.55; color:#0c4a6e; white-space:pre-wrap;">
  ...
  return `
    <div class="pdf-export-container" style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif; background:#f8fafc; color:#0f172a; padding:24px; max-width:900px; margin:0 auto;">
      <div style="background:linear-gradient(135deg, #0f172a 0%, #1e293b 100%); color:#ffffff; padding:28px 32px; border-radius:16px; margin-bottom:24px; text-align:center;">
        <h1 style="font-size:1.6rem; font-weight:800; margin-bottom:6px; letter-spacing:-0.02em;">${testTitle}</h1>
        <p style="font-size:0.95rem; color:#94a3b8; margin-bottom:16px;">Official Performance Score Report</p>
        <div style="display:inline-block; background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2); padding:6px 18px; border-radius:30px; font-size:0.85rem;">
          Student: <strong>${name}</strong> &nbsp;|&nbsp; Date: <strong>${dateStr}</strong>
        </div>
      </div>

      <div style="display:grid; grid-template-columns:repeat(4, 1fr); gap:12px; margin-bottom:24px;">
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; padding:16px; text-align:center;">
          <div style="font-size:0.75rem; font-weight:700; color:#64748b; text-transform:uppercase;">Math Score</div>
          <div style="font-size:1.8rem; font-weight:800; color:#0284c7; margin-top:4px;">${scoreDisplay}</div>
          <div style="font-size:0.75rem; color:#94a3b8;">Scaled (200-800)</div>
        </div>
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; padding:16px; text-align:center;">
          <div style="font-size:0.75rem; font-weight:700; color:#059669; text-transform:uppercase;">Correct</div>
          <div style="font-size:1.8rem; font-weight:800; color:#059669; margin-top:4px;">${correct}</div>
          <div style="font-size:0.75rem; color:#94a3b8;">${pct}% Accuracy</div>
        </div>
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; padding:16px; text-align:center;">
          <div style="font-size:0.75rem; font-weight:700; color:#dc2626; text-transform:uppercase;">Incorrect</div>
          <div style="font-size:1.8rem; font-weight:800; color:#dc2626; margin-top:4px;">${wrong}</div>
          <div style="font-size:0.75rem; color:#94a3b8;">Wrong Answers</div>
        </div>
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; padding:16px; text-align:center;">
          <div style="font-size:0.75rem; font-weight:700; color:#d97706; text-transform:uppercase;">Skipped</div>
          <div style="font-size:1.8rem; font-weight:800; color:#d97706; margin-top:4px;">${unattempted}</div>
          <div style="font-size:0.75rem; color:#94a3b8;">Unanswered</div>
        </div>
      </div>

      <div style="margin-bottom:20px;">
        <h2 style="font-size:1.2rem; font-weight:800; color:#0f172a; margin-bottom:12px;">Question Breakdown & Solutions</h2>
        ${qCards}
      </div>

      <div style="text-align:center; padding:16px; color:#94a3b8; font-size:0.8rem; border-top:1px solid #e2e8f0;">
        Generated by EduQuest Digital SAT Preparation Platform
      </div>
    </div>
  `;
}
...
const SUPABASE_URL = (typeof window !== 'undefined' && window.SUPABASE_URL) ? window.SUPABASE_URL : 'YOUR_SUPABASE_URL';
const SUPABASE_KEY = (typeof window !== 'undefined' && window.SUPABASE_KEY) ? window.SUPABASE_KEY : 'YOUR_SUPABASE_KEY';
  ...
  try {
    await fetch(`${SUPABASE_URL}/rest/v1/users`, {
      ...
      body: JSON.stringify({
        ...
        email: student.email,
        phone: student.phone,
        topic: 'Digital SAT Math Practice Test 1',
...
function startTest() {
  ...
  const phone = phoneVal ? (($('regCC') ? $('regCC').value : '') + ' ' + phoneVal).trim() : '';
  ...
  buildPalette();
  renderQ();
  startTimer(70 * 60); // 70 minutes (35 min per module)
...
function renderQ() {
  ...
  const modNum = q.id <= 27 ? 1 : 2;
  const modQNum = q.id <= 27 ? q.id : q.id - 27;

  $('qBadge').textContent = `Q ${q.id}`;
  $('qModuleTag').textContent = `Module ${modNum}`;
  $('qTopicTag').textContent = q.topic || 'Math';
  $('modName').textContent = `📐 Module ${modNum}: Math (Question ${modQNum} of 27)`;
...
async function doSubmit() {
  ...
  // SAT Math Section Scaled Score (200 - 800)
  const scaled = Math.min(800, Math.max(200, Math.round(200 + (correct/total)*600)));
...
function renderResults(result, saveRes) {
  ...
  const savedBadge = saveRes && saveRes.ok
    ...
    : `<div class="saved-badge" style="color:#dc2626;border-color:rgba(220,38,38,0.3);background:rgba(220,38,38,0.08)">⚠️ Save completed locally</div>`;

  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Digital SAT Math Practice Test 1</div>
      <div class="sat-score" style="color:${scoreColor}">${scaled}</div>
      <div class="score-line">Math Section Score · ${correct} / ${total} correct (${pct}%) · Grade: ${grade}</div>
    ...
    <div class="charts-row">
      <div class="chart-card"><h3>📊 Performance Breakdown</h3><canvas id="pieChart" height="220"></canvas></div>
    </div>
    ...
    <div class="q-review-section" style="margin-top:24px; text-align:left;">
      ...
      <div class="q-review-list">
        ${details.map((q, i) => {
          ...
          let imgSrc = '';
          if (q.useImage && q.imageKey && typeof QUESTION_IMAGES !== 'undefined' && QUESTION_IMAGES[q.imageKey]) {
            imgSrc = QUESTION_IMAGES[q.imageKey];
          }
          ...
          return `
            <div class="q-card" style="background:#fff; border:1px solid var(--border); border-radius:12px; padding:20px; margin-bottom:16px; box-shadow:var(--shadow);">
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                <div>
                  <span style="font-weight:700; font-size:0.95rem; font-family:'IBM Plex Mono',monospace;">Question ${q.id || (i+1)} (Module ${modNum} · Q${modQNum})</span>
                  <span style="margin-left:8px; font-size:0.75rem; background:rgba(124,58,237,0.08); color:var(--accent); padding:2px 8px; border-radius:10px;">${q.topic || 'Math'}</span>
                </div>
              ...
              ${imgSrc ? `<div style="text-align:center; margin-bottom:14px;"><img src="${imgSrc}" style="max-width:100%; max-height:260px; border-radius:8px;" alt="Question Image" /></div>` : ''}
              ...
              <div style="margin-top:12px; padding:12px 14px; background:rgba(124,58,237,0.05); border-left:3px solid var(--accent); border-radius:0 8px 8px 0; font-size:0.85rem; line-height:1.55; color:var(--ink2); white-space:pre-wrap;">
                <strong style="color:var(--accent); display:block; margin-bottom:4px;">💡 Solution & Explanation:</strong> 
...
function downloadPDF() {
  ...
  const opt = {
    margin: [10, 10, 10, 10],
    filename: `SAT_Math_MT1_Report_${sName.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`,
