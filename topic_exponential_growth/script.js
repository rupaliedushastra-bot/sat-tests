...
function generateCleanReportHTML(t, details, emailStr, studentNameStr) {
  ...
  const testTitle = t.exam_name || t.topic || 'Digital SAT - Exponential Growth & Decay';
  ...
  const total = t.total || (details ? details.length : 20);
        ...
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; padding:20px; margin-bottom:20px; box-shadow:0 2px 8px rgba(0,0,0,0.04); page-break-inside:avoid;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px solid #f1f5f9; padding-bottom:10px;">
            <div>
              ...
              <span style="display:inline-block; margin-left:8px; font-size:0.75rem; background:#f1f5f9; color:#475569; padding:2px 8px; border-radius:10px;">${d.topic || 'Exponential Models'}</span>
...
function startTest() {
  ...
  buildPalette();
  renderQ();
  startTimer(30 * 60); // 30 minutes for 20 questions
...
function renderQ() {
  ...
  $('qTopicTag').textContent = q.topic || 'Exponential Models';
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Exponential Growth & Decay</div>
            ...
            <div class="q-card" style="background:#fff; border:1px solid var(--border); border-radius:12px; padding:20px; margin-bottom:16px; box-shadow:var(--shadow);">
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                <div>
                  ...
                  <span style="margin-left:8px; font-size:0.75rem; background:rgba(124,58,237,0.08); color:var(--accent); padding:2px 8px; border-radius:10px;">${q.topic || 'Exponential Models'}</span>
...
function downloadPDF() {
  ...
  const opt = {
    margin: [10, 10, 10, 10],
    filename: `SAT_Exp_Growth_Report_${sName.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`,
