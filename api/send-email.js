    ...
    if (body.type === "access_request") {
      ...
      await transport.sendMail({
        ...
        html: `
          <div style="font-family:sans-serif;max-width:620px;margin:0 auto;padding:28px;background:#0B1C3D;color:#f1f5ff;border-radius:16px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
              <h2 style="color:#d97706;margin:0;">🔒 Exam Access Request</h2>
              <span style="background:#d9770622;border:1px solid #d9770655;color:#fbbf24;padding:3px 10px;border-radius:50px;font-size:11px;font-weight:700;">REQUEST</span>
            </div>
            <hr style="border-color:rgba(255,255,255,.1);margin-bottom:20px"/>
            <table style="width:100%;border-collapse:collapse;">
              <tr>
                <td style="padding:8px 0;color:rgba(241,245,255,.5);font-size:13px;width:180px;">Student Email</td>
                <td style="padding:8px 0;font-weight:600;font-size:13px;">${userEmail}</td>
              </tr>
              <tr>
                <td style="padding:8px 0;color:rgba(241,245,255,.5);font-size:13px;">Request Time</td>
                <td style="padding:8px 0;font-weight:600;font-size:13px;">${new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })} IST</td>
              </tr>
            </table>
            <div style="margin-top:24px;padding:16px;background:rgba(217,119,6,.07);border:1px solid rgba(217,119,6,.2);border-radius:10px;font-size:12px;color:rgba(241,245,255,.5);">
              The student is waiting for you to approve their exam access. You can grant access from the Admin Dashboard.
            </div>
            <hr style="border-color:rgba(255,255,255,.08);margin:20px 0 12px"/>
            <p style="color:rgba(241,245,255,.25);font-size:11px;text-align:center;margin:0;">EduQuest SAT Practice Portal · rupali.eduquest@gmail.com</p>
          </div>
      ...
      return res.status(200).json({ ok: true, message: "Access request notification sent" });
    }

    return res.status(400).json({ error: "Invalid email type. Use 'signup', 'result', or 'access_request'." });
