  ...
  try {
    await fetch(`${SUPABASE_CONFIG.url}/rest/v1/sat_topic_user`, {
      ...
      body: JSON.stringify({
        ...
        email: student.email,
        phone: student.phone,
        topic: 'PDF - Text Structure and Purpose',
...
function renderQ() {
  ...
  if ($('qTopic')) $('qTopic').textContent = q.topic || 'Text Structure and Purpose';
  ...
  const qTextEl = $('qText');
  qTextEl.innerHTML = q.text; // Using innerHTML to render HTML tables in text
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Text Structure and Purpose  · Topic Test</div>
...
function downloadPDF() {
  ...
  html2pdf().set({
    margin: [10, 10, 10, 10],
    filename: `EduQuest_Report_${student.name.replace(/[^a-zA-Z0-9]/g, '_')}_PDF_Text_Structure_and_Purpose.pdf`,
