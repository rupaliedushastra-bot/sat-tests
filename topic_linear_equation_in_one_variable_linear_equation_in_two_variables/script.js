...
// For the sake of this mock, we assume the user already configured supabase in dashboard or we will use the global one if needed.
// Actually, to make it work out of the box, we will use window.supabase or initialize it correctly.
// I will copy the supabase config from another test script.

let currentQuestion = 0;
let userAnswers = new Array(QUESTIONS.length).fill(null);
let reviewMarked = new Array(QUESTIONS.length).fill(false);

const topicName = "Linear Equation In One Variable Linear Equation In Two Variables";
...
async function submitTest() {
  ...
  // Read supabase config from localStorage or fallback
  ...
  if (savedUrl && savedKey) {
    ...
    console.warn("Supabase not configured in localStorage. Result not saved.");
  }
