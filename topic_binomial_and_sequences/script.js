...
function generateCleanReportHTML(t, details, emailStr, studentNameStr) {
  ...
  const testTitle = t.exam_name || t.topic || 'Digital SAT - Binomial Theorem & Sequences';
  ...
  const total = t.total || (details ? details.length : 30);
        ...
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; padding:20px; margin-bottom:20px; box-shadow:0 2px 8px rgba(0,0,0,0.04); page-break-inside:avoid;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; border-bottom:1px solid #f1f5f9; padding-bottom:10px;">
            <div>
              ...
              <span style="display:inline-block; margin-left:8px; font-size:0.75rem; background:#f1f5f9; color:#475569; padding:2px 8px; border-radius:10px;">${d.topic || 'Binomial & Sequences'}</span>
              <span style="display:inline-block; margin-left:4px; font-size:0.72rem; background:rgba(124,58,237,0.1); color:#7c3aed; padding:2px 8px; border-radius:10px; font-weight:600;">${d.difficulty || 'MODERATE'}</span>
            </div>
            ${statusBadge}
          </div>
  ...
  return `
    <div class="pdf-export-container" style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif; background:#f8fafc; color:#0f172a; padding:24px; max-width:900px; margin:0 auto;">
      <div style="background:linear-gradient(135deg, #0f172a 0%, #1e293b 100%); color:#ffffff; padding:28px 32px; border-radius:16px; margin-bottom:24px; text-align:center;">
        ...
        <p style="font-size:0.95rem; color:#94a3b8; margin-bottom:16px;">Performance Score Report</p>
      ...
      <div style="display:grid; grid-template-columns:repeat(4, 1fr); gap:12px; margin-bottom:24px;">
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:12px; padding:16px; text-align:center;">
          <div style="font-size:0.75rem; font-weight:700; color:#64748b; text-transform:uppercase;">Scaled Score</div>
      ...
      <div style="margin-bottom:20px;">
        <h2 style="font-size:1.2rem; font-weight:800; color:#0f172a; margin-bottom:12px;">Detailed Solutions & Explanations</h2>
        ${qCards}
      </div>

      <div style="text-align:center; padding:16px; color:#94a3b8; font-size:0.8rem; border-top:1px solid #e2e8f0;">
        Generated by EduQuest Digital SAT Platform
      </div>
    </div>
  `;
}
  ...
  try {
    await fetch(`${SUPABASE_URL}/rest/v1/users`, {
      ...
      body: JSON.stringify({
        ...
        email: student.email,
        phone: student.phone,
        topic: 'Binomial Theorem & Sequences',
...
function startTest() {
  ...
  buildPalette();
  renderQ();
  startTimer(45 * 60); // 45 minutes for 30 questions
...
function renderQ() {
  ...
  const diffTag = $('qDiffTag');
  if (diffTag) {
    diffTag.textContent = q.difficulty || 'MODERATE';
    diffTag.className = 'difficulty-tag ' + (
      q.difficulty === 'EASY' ? 'diff-easy' : 
      q.difficulty === 'SUPER TOUGHEST' ? 'diff-super' : 'diff-moderate'
    );
  }
...
function renderResults(result, saveRes) {
  ...
  wrap.innerHTML = `
    <div class="res-hero">
      <div style="font-size:0.8rem;opacity:.55;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;position:relative">Binomial Theorem & Sequences</div>
      <div class="sat-score" style="color:${scoreColor}">${scaled}</div>
      <div class="score-line">Topic Score · ${correct} / ${total} correct (${pct}%) · Grade: ${grade}</div>
            ...
            <div class="q-card" style="background:#fff; border:1px solid var(--border); border-radius:12px; padding:20px; margin-bottom:16px; box-shadow:var(--shadow);">
              <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                <div>
                  ...
                  <span style="margin-left:4px; font-size:0.72rem; background:rgba(0,0,0,0.05); color:#64748b; padding:2px 8px; border-radius:10px;">${q.difficulty || 'MODERATE'}</span>
                </div>
...
function downloadPDF() {
  ...
  const opt = {
    margin: [10, 10, 10, 10],
    filename: `SAT_Binomial_Sequences_Report_${sName.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`,
